...
1. The retry lives in the HTTP client and in the job. Do we keep both?
   → Recommendation: no — drop the one in the job, the client already
     owns the policy.
   → Tradeoff: one place to tune, but the job loses its own backoff
...
- MUST: One checkbox line per task: `- [ ] T-N — title — repo: <repo> — blocked by: T-M — commit: —`. `/implement` flips the checkbox and fills the commit reference.
